...
>> This could be anywhere from 2 to 6. $680 is inside the second class, so it will be bigger than the 2 observations in the first class. But we don't know its relative position inside the second class. If it is the smallest number, then only the 2 observation in the first class will be lower than 680, but if it is the biggest number in the second class, 4+2 observartions could be smaller than 680.  

> c.  How would you describe the relationship between the income $(\mathrm{x})$ and housing $(y)$?
